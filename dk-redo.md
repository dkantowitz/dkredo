...
dk-ifchange <label>                # label only — check deps from existing stamp
```
...
**Label-only mode:** When called with only a label and no inputs,
`dk-ifchange` reads the dependency list from the existing stamp file and
checks those files for changes. If no stamp exists, it exits 0 (changed —
first run, needs build). This is useful when the full dependency list is
discovered during the build (e.g., by gcc `-MD`) and recorded by `dk-stamp`.

**Union behavior:** When called with both a label and inputs, `dk-ifchange`
checks the **union** of the provided inputs and any files recorded in the
existing stamp. This ensures that dependencies discovered during a previous
build (e.g., header files found by gcc) are still checked even if they are
not in the current argument list. New inputs not in the stamp are treated as
changed. Files in the stamp but not in the current arguments are still
checked for content changes.

Because globs are expanded by the shell before dk-redo sees them, adding or
removing files that match the glob changes the argument list, which triggers
a rebuild. Each file's facts (size, blake3 hash, existence) are checked — if
any fact no longer holds, the inputs are considered changed. Size is checked
first as a fast path to avoid hashing.
...
dk-stamp <label> -M <depfile>              # read gcc/clang makefile dep format
dk-stamp <label> src/*.c -M .deps/out.d    # positional args + depfile combined
```
...
firmware:
    ...
    dk-stamp firmware.bin src/*.c include/*.h libs/*.a -M .deps/firmware.d
```

The `-M` flag merges gcc's discovered headers directly. Or with `--append`:

```just
    dk-stamp firmware.bin src/*.c include/*.h libs/*.a
    dk-stamp --append firmware.bin -M .deps/firmware.d
```

By using `--append` or `-M`, the final stamp contains the union: gcc's
discovered headers from the dep file AND the explicit source/lib inputs.

**With `-M <depfile>`, `dk-stamp` reads a makefile-format dependency file**
(as generated by `gcc -MD -MF` or `clang -MD -MF`) and extracts the
dependency paths. The depfile format is:
...
`dk-stamp` parses the file, strips the `target:` prefix, handles
backslash-newline continuations, and extracts all dependency paths. These
paths are merged with any other inputs (positional args, stdin). The target
name in the depfile is ignored — the stamp label comes from the command line.

This replaces the common pattern of shell-parsing `.d` files:

```just
# before: fragile shell parsing
dk-stamp firmware.bin - < <(sed 's/^[^:]*://; s/\\$//' .deps/out.d | xargs -n1)

# after: native depfile support
dk-stamp firmware.bin -M .deps/out.d
```

`-M` can be combined with positional args, stdin, and `--append`:

```just
dk-stamp firmware.bin src/*.c -M .deps/out.d        # args + depfile
dk-stamp --append firmware.bin -M .deps/out.d        # merge depfile into existing stamp
```
...
| `-M file`  | Read dependencies from a makefile-format dep file (gcc `-MD`) |
