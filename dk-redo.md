...
dk-redo is a single script. Symlinks provide the short command names.
...
| ---- | -------- | -------------------------------------------------------- |
| `-v` | minimum  | Verbose: print which files changed                       |
| `-n` | good     | Dry run: report changed/unchanged without updating state |
| `-q` | minimum  | Quiet: suppress "up to date" message                     |

#### `dk-stamp` — record current input state

```
dk-stamp <label> [inputs...]
dk-stamp <label> -                 # stdin, same modes as dk-ifchange
dk-stamp <label> -0
```
...
The first `dk-stamp --append` records gcc's discovered headers. The final
`dk-stamp` (no `--append`) replaces with the complete, authoritative list.
...
| Flag       | Category | Description                                      |
| ---------- | -------- | ------------------------------------------------ |
| `-v`       | minimum  | Verbose: print stamp path and hash               |
| `-q`       | minimum  | Quiet: no output on success                      |
| `--append` | good     | Merge into existing stamp instead of replacing it |
...
| Flag    | Category | Description                  |
| ------- | -------- | ---------------------------- |
| `--all` | good     | Remove all stamps            |
| `-v`    | minimum  | Verbose: list removed stamps |
...
- `0` — at least one label is out of date (or no stamps exist)
...
| Flag     | Category | Description                 |
| -------- | -------- | --------------------------- |
| `-v`     | minimum  | Show hash details per label |
| `-q`     | good     | Just exit code, no output   |
| `--json` | fancy    | Output as JSON array        |
...
| Flag     | Category | Description                           |
| -------- | -------- | ------------------------------------- |
| `-v`     | minimum  | Show which input triggered each label |
| `--json` | fancy    | Output as JSON                        |
...
| Flag   | Category | Description                              |
| ------ | -------- | ---------------------------------------- |
| `--lr` | good     | Left-to-right layout (default: top-down) |
...
`dk-affects`) support three input modes. They can be combined freely.
...
[script('bash')]
engine:
    fd -e rs src | dk-ifchange target/release/engine - || exit 0
    cargo build --release
    fd -e rs src | dk-stamp target/release/engine -
```

> Note: stdin mode requires a `[script]` recipe (single shell process) so
> the pipe works. The `|| exit 0` replaces the `?` sigil since guard sigils
> operate on individual lines, not piped commands.
...
On first run, `data/app.db` doesn't exist — no content to hash, can't match
any stamp — so `dk-ifchange` returns 0 (changed) and the recipe runs. After
`dk-stamp`, subsequent runs skip unless `schema.sql` changes.
...
that file *staying absent*. If someone creates `./include/config.h`, it
...
files record resolved paths of files that *were* included — not paths that
...
**Hybrid approach for precision + coverage.** Combine gcc's dep output
(catches changes to individual headers, including system headers) with
directory hashing (catches new files appearing in search paths):

```just
firmware:
    ?dk-ifchange firmware.bin include/ - < .deps/firmware.d
    gcc -MD -MF .deps/firmware.d -o firmware.bin src/*.c -Iinclude/
    dk-stamp firmware.bin include/ - < .deps/firmware.d
```

The directory argument covers the negative-dependency gap that gcc's dep
output misses. This is arguably more robust than redo's `redo-ifcreate`,
which requires the developer to manually identify and declare each
absent-file dependency.
